...
#include "intervention_msgs/action/move_robot_tf.hpp"
//Logging
#include "interventions/logger.hpp"
//Behavior tree
#include "behaviortree_cpp/action_node.h"

class MoveRobotTf : public BT::StatefulActionNode
{
public: 
    using MoveAction = intervention_msgs::action::MoveRobotTf;
    using GoalHandleMoveAction = rclcpp_action::ClientGoalHandle<MoveAction>;

    /**
     * @brief Constructor for behavior tree node 
    */
    MoveRobotTf(const std::string &name, const BT::NodeConfig &config) : StatefulActionNode(name, config)
    {
        initRos();
    } 
    ...
    */   
    ...
    {
        return{BT::InputPort<std::string>("goalFrame"),
               BT::InputPort<std::string>("referenceFrame"),
               BT::InputPort<std::string>("eeFrame"),
...
private:
    ...
    */
    ...
    std::string goalFrame, referenceFrame, eeFrame;
