#include "interventions/behavior_tree/action_servers/move_robot_named_action_server.hpp"

using namespace std::placeholders;
using namespace std::chrono_literals;

MoveRobotNamedActionServer::MoveRobotNamedActionServer(const std::shared_ptr<rclcpp::Node> node)
{
    node_ = node;
    ...
    action_lifeline_publisher_ = node_->create_publisher<std_msgs::msg::Int8>("move_robot_named_action_lifeline", 1);
    ...
    std::stringstream ss;
    ss << "MOVE ROBOT NAMED FRAME ACTION SERVER INITIALISATION\n";
    ...
    action_lifeline_timer_ = node_->create_wall_timer(std::chrono::milliseconds(tree_frequency_), std::bind(&MoveRobotNamedActionServer::lifeline, this));

    //Create action server
    this->action_server_ = rclcpp_action::create_server<MoveAction>(
        node_, 
        action_name_,
        std::bind(&MoveRobotNamedActionServer::handleGoal, this, _1, _2),
        std::bind(&MoveRobotNamedActionServer::handleCancel, this, _1),
        std::bind(&MoveRobotNamedActionServer::handleAccepted, this, _1));
}

rclcpp_action::GoalResponse MoveRobotNamedActionServer::handleGoal(
    const rclcpp_action::GoalUUID &uuid,
    std::shared_ptr<const MoveAction::Goal> goal)
{   
    //Check if the input is valid 
    //Check if the named target exists
    std::vector<std::string> targets = move_group_->getNamedTargets();
    if(!(std::find(targets.begin(), targets.end(), goal->named_goal) != targets.end()))
    {
        std::stringstream ss;
        ss << "Goal rejected as the named target pose: " << goal->named_goal << " could not be found \n";
...
rclcpp_action::CancelResponse MoveRobotNamedActionServer::handleCancel(
...
void MoveRobotNamedActionServer::handleAccepted(
    const std::shared_ptr<GoalHandleMoveAction> goal_handle)
{
    std::thread{std::bind(&MoveRobotNamedActionServer::execute, this, _1), goal_handle}.detach();
}

void MoveRobotNamedActionServer::execute(const std::shared_ptr<GoalHandleMoveAction> goal_handle)
{
    ...
    std::thread{std::bind(&MoveRobotNamedActionServer::execute_move, this, _1, _2, _3), 
                goal,
                success,
                stop}.detach();
...
void MoveRobotNamedActionServer::lifeline()
{
...
void MoveRobotNamedActionServer::execute_move(const std::shared_ptr<const intervention_msgs::action::MoveRobotNamed_Goal> goal,
...
int main(int argc, char** argv)
{
    ...
    std::shared_ptr<rclcpp::Node> node = std::make_shared<rclcpp::Node>("move_robot_named_action_server_node");
    MoveRobotNamedActionServer mrtf(node);
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}