...
BT::NodeStatus TestB::onStart()
{
    ...
    send_goal_options.goal_response_callback = std::bind(&TestB::goalResponseCallback, this, _1);
    send_goal_options.feedback_callback = std::bind(&TestB::feedbackCallback, this, _1, _2);
    send_goal_options.result_callback = std::bind(&TestB::resultCallback, this, _1);
    ...
    action_watchdog_timer_ = node_->create_wall_timer(std::chrono::milliseconds(tree_frequency_),
                                                std::bind(&TestB::actionWatchdogCallback, this));
    return BT::NodeStatus::RUNNING;
}

BT::NodeStatus TestB::onRunning()
{
...
void TestB::initRos()
{
    ...
    init_error_ = false;
    node_ = std::make_shared<rclcpp::Node>("testB_bt_node");
    auto qos = rclcpp::QoS(rclcpp::KeepLast(1), rmw_qos_profile_sensor_data);
    spinThread_ = std::make_shared<std::thread>(std::bind(&TestB::spinNode, this));
    ...
    if(timeout_ <= 0)
    {
        error_message_ = "No valid timeout specified";
        logger_->logError(node_, error_message_);
        init_error_ = true;
        return;
    }
    ...
    std::stringstream ss;
    ss << "INTERVENTION B BT NODE INITIALISATION\n";
    ...
    action_lifeline_subscriber_ = node_->create_subscription<std_msgs::msg::Int8>("testB_action_lifeline", qos, std::bind(&TestB::actionLifeLineCallback, this, _1));
}

void TestB::goalResponseCallback(const GoalHandleTestAction::SharedPtr &goal_handle)
{
    if(!goal_handle)
    {
...
void TestB::feedbackCallback(GoalHandleTestAction::SharedPtr, const std::shared_ptr<const TestAction::Feedback> feedback)
{
  latest_feedback_ = feedback;
}
void TestB::resultCallback(const GoalHandleTestAction::WrappedResult &result)
{
    switch (result.code)
    {
...
void TestB::actionWatchdogCallback()
{
...
void TestB::actionLifeLineCallback(const std_msgs::msg::Int8::SharedPtr msg)
{
