...
BT::NodeStatus MoveRobotTf::onStart()
{
    ...
    getInput("goalFrame", goalFrame);
    getInput("referenceFrame", referenceFrame);
    getInput("eeFrame", eeFrame);
    ...
    goal_msg.goal_frame = goalFrame;
    goal_msg.reference_frame = referenceFrame;
    goal_msg.ee_frame = eeFrame;
    ...
    send_goal_options.goal_response_callback = std::bind(&MoveRobotTf::goalResponseCallback, this, _1);
    send_goal_options.feedback_callback = std::bind(&MoveRobotTf::feedbackCallback, this, _1, _2);
    send_goal_options.result_callback = std::bind(&MoveRobotTf::resultCallback, this, _1);
    ...
    action_watchdog_timer_ = node_->create_wall_timer(std::chrono::milliseconds(tree_frequency_),
                                                std::bind(&MoveRobotTf::actionWatchdogCallback, this));
    return BT::NodeStatus::RUNNING;
}

BT::NodeStatus MoveRobotTf::onRunning()
{
...
void MoveRobotTf::onHalted()
{
    client_ptr_->async_cancel_all_goals();
    logger_->logWarning(node_, "BT node halted");
}

void MoveRobotTf::initRos()
{
    action_watchdog_counter_ = 0;
    init_error_ = false;
    node_ = std::make_shared<rclcpp::Node>("move_robot_tf_bt_node");
    auto qos = rclcpp::QoS(rclcpp::KeepLast(1), rmw_qos_profile_sensor_data);
    spinThread_ = std::make_shared<std::thread>(std::bind(&MoveRobotTf::spinNode, this));
    ...
    std::stringstream ss;
    ss << "MOVE ROBOT TF BT NODE INITIALISATION\n";
    ...
    action_lifeline_subscriber_ = node_->create_subscription<std_msgs::msg::Int8>("move_robot_tf_action_lifeline", qos, std::bind(&MoveRobotTf::actionLifeLineCallback, this, _1));
}

void MoveRobotTf::goalResponseCallback(const GoalHandleMoveAction::SharedPtr &goal_handle)
{
    if(!goal_handle)
    {
...
void MoveRobotTf::feedbackCallback(GoalHandleMoveAction::SharedPtr, const std::shared_ptr<const MoveAction::Feedback> feedback)
{
  latest_feedback_ = feedback;
}

void MoveRobotTf::resultCallback(const GoalHandleMoveAction::WrappedResult &result)
{
    switch (result.code)
    {
...
void MoveRobotTf::spinNode()
{
    rclcpp::spin(node_);
}

void MoveRobotTf::actionWatchdogCallback()
{
...
void MoveRobotTf::actionLifeLineCallback(const std_msgs::msg::Int8::SharedPtr msg)
{
