...
BT::NodeStatus FinalMoveRobot::onStart()
{
    ...
    send_goal_options.goal_response_callback = std::bind(&FinalMoveRobot::goalResponseCallback, this, _1);
    send_goal_options.feedback_callback = std::bind(&FinalMoveRobot::feedbackCallback, this, _1, _2);
    send_goal_options.result_callback = std::bind(&FinalMoveRobot::resultCallback, this, _1);
    ...
    action_watchdog_timer_ = node_->create_wall_timer(std::chrono::milliseconds(tree_frequency_),
                                                std::bind(&FinalMoveRobot::actionWatchdogCallback, this));
    return BT::NodeStatus::RUNNING;
}

BT::NodeStatus FinalMoveRobot::onRunning()
{
...
void FinalMoveRobot::onHalted()
{
    client_ptr_->async_cancel_all_goals();
    logger_->logWarning(node_, "BT node halted");
}

void FinalMoveRobot::initRos()
{
    action_watchdog_counter_ = 0;
    init_error_ = false;
    node_ = std::make_shared<rclcpp::Node>("final_move_robot_bt_node");
    auto qos = rclcpp::QoS(rclcpp::KeepLast(1), rmw_qos_profile_sensor_data);
    spinThread_ = std::make_shared<std::thread>(std::bind(&FinalMoveRobot::spinNode, this));
    ...
    std::stringstream ss;
    ss << "FINAL MOVE ROBOT BT NODE INITIALISATION\n";
    ...
    action_lifeline_subscriber_ = node_->create_subscription<std_msgs::msg::Int8>("move_robot_joint_state_action_lifeline", qos, std::bind(&FinalMoveRobot::actionLifeLineCallback, this, _1));
}

void FinalMoveRobot::goalResponseCallback(const GoalHandleMoveAction::SharedPtr &goal_handle)
{
    if(!goal_handle)
    {
...
void FinalMoveRobot::feedbackCallback(GoalHandleMoveAction::SharedPtr, const std::shared_ptr<const MoveAction::Feedback> feedback)
{
  latest_feedback_ = feedback;
}

void FinalMoveRobot::resultCallback(const GoalHandleMoveAction::WrappedResult &result)
{
    switch (result.code)
    {
...
void FinalMoveRobot::spinNode()
{
    rclcpp::spin(node_);
}

void FinalMoveRobot::actionWatchdogCallback()
{
...
void FinalMoveRobot::actionLifeLineCallback(const std_msgs::msg::Int8::SharedPtr msg)
{
